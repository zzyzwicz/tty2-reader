...
RSpec.describe TTY2::Reader::KeyEvent, "#from" do
  let(:keys) { TTY2::Reader::Keys.keys }
  ...
  it "parses ctrl-a to ctrl-z inputs" do
    (1..26).zip("a".."z").each do |code, char|
      event = described_class.from(TTY2::Reader::Keys.ctrl_keys, code.chr)
  ...
  it "exposes Line object" do
    event = described_class.from(keys, "c", "ab")
    expect(event.line.to_s).to eq("ab")
  end
