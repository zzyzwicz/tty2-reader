...
RSpec.describe TTY2::Reader, "complete word" do
  ...
  let(:completion_handler) {
    ->(word, context) { @completions.grep(/\A#{Regexp.escape(word)}/) }
  }
  let(:options) {
  ...
  it "adds space suffix to full word completion on the first tab" do
  ...
  it "skips adding space suffix for partial word completion on the first tab" do
    @completions = %w[aa ab ac]
    options[:completion_suffix] = " "
    reader = described_class.new(**options)
    input << "x " << "a" << "\t" << "\t" << "\n"
